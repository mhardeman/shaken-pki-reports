...
func PrintCertificateFindings(w io.Writer, r *CertificateGroupReport) {
	...
	fmt.Fprintf(w, "- %d certificates in the candidate corpus were excluded because they were not valid during the target validity period\n", r.SkippedExpiredAmount)
	fmt.Fprintf(w, "- %d certificates in the candidate corpus were excluded because they did not chain to program trust anchors\n", r.SkippedUntrustedAmount)
	fmt.Fprintf(w, "- %d valid certificates were tested against the remaining rules\n", r.TestedAmount)
	fmt.Fprintf(w, "- %0.2f issues on average found in valid but non-compliant certificates\n", r.AverageCertificatesWithProblems())
	fmt.Fprintf(w, "- %0.2f%% of valid certificates contain one or more Error level issue\n", r.AverageErrors())
	fmt.Fprintf(w, "- %0.2f%% of valid certificates contain one or more Warning level issue\n", r.AverageWarns())
	fmt.Fprintf(w, "- %0.2f%% of valid certificates contain one or more Notice level issue\n", r.AverageNotices())
	fmt.Fprintf(w, "- %0.2f%% of valid certificates are too old to be assessed against currently enforced expectations\n", r.AverageNotEffective())
	fmt.Fprintf(w, "- %0.0f days is the average remaining validity of the valid certificates\n", r.AverageRemainingValidity())
	fmt.Fprintf(w, "- %0.0f days is the average initial validity of the valid certificates\n", r.AverageInitialValidity())
